...
public class QuantumComputations
{
    ...
    {
        SimpleQuantumExecutor.LOG.finer(s);
    }


    public static Complex[][] calculateStepMatrix(List<QuantumGate> gates, int numberOfQubits, QuantumExecutor qee)
    {
        long l0 = System.currentTimeMillis();
        Complex[][] a = new Complex[1][1];
        a[0][0] = Complex.ONE;
        int idx = numberOfQubits - 1;
        while(idx >= 0)
        {
            final int cnt = idx;
            QuantumGate myGate = gates.stream()
                                      .filter(gate -> gate.getHighestAffectedQubitIndex() == cnt)
                                      .findFirst()
    ...
    {
        ArrayList<QuantumStep> answer = new ArrayList<>();
        answer.add(s);
        if(s.getType() == QuantumStepType.PSEUDO)
        {
            s.setComplexStep(s.getIndex());
            return answer;
        }
        List<QuantumGate> gates = s.getGates();
        if(gates.isEmpty())
        {
            return answer;
        }
        ...
        QuantumGate targetGate = gates.get(0);
        ...
        List<QuantumGate> firstGates = new ArrayList<>();
            ...
            else if(gate instanceof TwoQubitGate)
            {
                ...
                else
                {
                    ...
                    if(first > second)
                    {
                        ...
                        QuantumStep prePermutation = new QuantumStep(pg);
                        QuantumStep postPermutation = new QuantumStep(pg);
                    ...
                    else
                    {
                        ...
                        QuantumStep prePermutation = new QuantumStep(pg);
                        QuantumStep prePermutationInv = new QuantumStep(pg);
                        int realStep = s.getIndex();
                        s.setComplexStep(-1);
                        answer.add(0, prePermutation);
                        answer.add(prePermutationInv);
                        QuantumStep postPermutation = new QuantumStep();
                        QuantumStep postPermutationInv = new QuantumStep();
                        if(first != second - 1)
                        {
    ...
    public static int fraction(int p, int max)
    {
        ...
        int offset = length;
        int dim = 1 << offset;
        double r = (double)p / dim + 0.000001;
        return QuantumComputations.fraction(r, max);
    }
    ...
    private static Complex[] getNextProbability(List<QuantumGate> gates, Complex[] v)
    {
    ...
    private static Complex[] getNextProbability2(List<QuantumGate> gates, Complex[] v)
    {
        QuantumGate gate = gates.get(0);
        ...
        if(gates.size() > 1)
        {
            ...
            List<QuantumGate> nextGates = gates.subList(1, gates.size());
            boolean id = true;
            for(QuantumGate g : nextGates)
            {
     ...
     */
    private static void validateGates(List<QuantumGate> gates, int nQubits)
    {
        for(QuantumGate gate : gates)
        {
    ...
    private static List<QuantumGate> getAllGates(List<QuantumGate> gates, int nQubits)
    {
        validateGates(gates, nQubits);
        dbg("getAllGates, orig = " + gates);
        List<QuantumGate> answer = new ArrayList<>();
        int idx = nQubits - 1;
        while(idx >= 0)
        {
            final int cnt = idx;
    ...
    private static void processBlockGate(ControlledBlockGate gate, ArrayList<QuantumStep> answer)
    {
        QuantumStep master = answer.get(answer.size() - 1);
        ...
        for(int i = 0; i < perm.size(); i++)
        {
            ...
            QuantumStep lpg = new QuantumStep(pg);
            if(i < perm.size() - 1)
            {
            ...
            answer.add(lpg);
            answer.add(0, new QuantumStep(pg));
        }
    }
     ...
     *
     * @param vectorresult an array of {@link Complex} objects
     ...
     */
    static Complex[] doImmediateMeasurement(List<QuantumGate> gates, Complex[] vector, int length)
    {
